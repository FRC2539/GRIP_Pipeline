...
void GripPipeline::process(cv::Mat source0){
	//Step Normalize0:
	//input
	cv::Mat normalizeInput = source0;
	int normalizeType = NORM_INF;
	double normalizeAlpha = 75.0;  // default Double
	double normalizeBeta = 0.0;  // default Double
	normalize(normalizeInput, normalizeType, normalizeAlpha, normalizeBeta, this->normalizeOutput);
	//Step Blur0:
	//input
	cv::Mat blurInput = normalizeOutput;
	BlurType blurType = BlurType::BILATERAL;
	double blurRadius = 0.7547169811320771;  // default Double
	blur(blurInput, blurType, blurRadius, this->blurOutput);
	//Step HSV_Threshold0:
	//input
	cv::Mat hsvThresholdInput = blurOutput;
	double hsvThresholdHue[] = {0.0, 97.16577540106951};
	double hsvThresholdSaturation[] = {0.0, 9.442289792119112};
	double hsvThresholdValue[] = {74.43502824858759, 93.63636363636364};
	...
	double filterContoursMinArea = 80.0;  // default Double
	double filterContoursMinPerimeter = 0.0;  // default Double
	double filterContoursMinWidth = 2.0;  // default Double
	...
	double filterContoursSolidity[] = {40.45197740112995, 100.0};
 ...
 */
cv::Mat* GripPipeline::getnormalizeOutput(){
...
std::vector<std::vector<cv::Point> >* GripPipeline::getfilterContoursOutput(){
	 ...
	 */
	void GripPipeline::normalize(cv::Mat &input, int type, double min, double max, cv::Mat &output) {
	   cv::normalize(input, output, min, max, type);
	}
